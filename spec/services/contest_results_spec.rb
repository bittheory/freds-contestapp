...
describe ContestResults do
	...
	context "initialize" do
		it "should raise exeception if not an array" do
			expect {ContestResults.new([1])}.to_not_raise_error
			expect {ContestResults.new([1,2,3,4])}.to_not_raise_error
			expect {ContestResults.new(["a","b"])}.to_not_raise_error
			expect {ContestResults.new("")}.to_raise_error
			expect {ContestResults.new(nil)}.to_raise_error
			expect {ContestResults.new([])}.to_raise_error
			expect {ContestResults.new}.to_raise_error
		end
	end
