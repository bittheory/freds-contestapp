class DashboardController < ApplicationController
  def index
  	# Load the contest results in reverse order
  	@contests = Contest.order("created_at desc")

  	#Instantiate a new Contest so the form loads correctly
  	@contest = Contest.new
  ...
  def create_contest
  	...
  	respond_to do |format|
  		if @contest.save
  			...
  			format.html { redirect_to root_path, notice: "#{@contest.name} Contest Winner: <a href='#{ order_path(@contest.order) }'>#{ @contest.order.email }</a>" }
